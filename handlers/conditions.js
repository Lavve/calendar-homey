...
const onEventAutocomplete = async (app, query, args, type) => {
  ...
  if (type === 'event') {
    if (query && query !== '') {
      ...
      return Promise.resolve(getEventList(app, filtered))
    }

    return Promise.resolve(getEventList(app, app.variableMgmt.calendars))
  }
...
const getEventList = (app, calendars) => {
  const eventList = []
...
const checkEvent = async (app, args, state, type) => {
  let filteredEvents
...
module.exports = async app => {
  ...
  cards.forEach(({ id, runListenerId, autocompleteListener }) => {
    ...
    conditionCard.registerRunListener((args, state) => checkEvent(app, args, state, runListenerId))
    if (autocompleteListener.argumentId && autocompleteListener.id) conditionCard.registerArgumentAutocompleteListener(autocompleteListener.argumentId, (query, args) => onEventAutocomplete(app, query, args, autocompleteListener.id)) // registerAutocompleteListener is async in the docs...
  })
}
